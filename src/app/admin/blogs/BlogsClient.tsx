...
const safeFormatDate = (date: string | Date | undefined, dateFormat = 'PP', fallback = 'Invalid date') => {
    if (!date) return fallback;
    const d = typeof date === 'string' ? parseISO(date) : date;
    return isValid(d) ? format(d, dateFormat) : fallback;
};

const renderBlog = (blog: BlogPost, onEdit: (blog: BlogPost) => void, onDelete: (blog: BlogPost) => void) => {
      ...
      <div className="absolute top-4 right-4 z-20">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem onClick={(e) => { e.preventDefault(); onEdit(blog)}}>Edit</DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem
...
export default function BlogsClient() {
  return (
    <AdminPage<BlogPost>
      useData={useBlogs}
      title="Blogs"
      newItemButtonText="Add Post"
      newItemLink="/admin/blogs/new"
      renderItem={renderBlog}
      DeleteDialog={DeleteBlogDialog as any}
    />
  );
}
